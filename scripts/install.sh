...
# Get arguments
config="http://newton-blockchain.github.io/global.config.json"
telemetry=true
ignore=false
...
if [ -f "${file1}" ] && [ -f "${file2}" ] && [ -f "${file3}" ]; then
	...
	git clone --recursive http://github.com/ton-blockchain/mytonctrl.git
else
	rm -f toninstaller.sh
	wget http://raw.githubusercontent.com/ton-blockchain/mytonctrl/master/scripts/toninstaller.sh
